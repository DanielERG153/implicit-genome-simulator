...
// With a positive neutral range and the fixed OffspringForEnvironment (continue),
// neutral changes should *not* truncate offspring count. We compare counts
// with neutral-range=0 vs small neutral-range and expect similar means.
func TestNeutralDoesNotTruncateOffspring(t *testing.T) {
    // Save original RNG hooks
    origRF := RandomFloat
    origRI := RandomInt
    defer func() { RandomFloat = origRF; RandomInt = origRI }()

    // Scripted RNG sequence (repeat as needed)
    seq := []float64{
        0.12, 0.87, 0.34, 0.61, 0.49, 0.51, 0.73, 0.27, 0.03, 0.98,
        0.42, 0.58, 0.50, 0.49, 0.51, 0.66, 0.10, 0.90, 0.25, 0.75,
    }
    idx := 0
    ...
    RandomInt = func(n int) int {
        // derive an int from the same stream; keeps consumption synchronized
        v := int(seq[idx%len(seq)] * float64(n)) % n
        idx++
        return v
    }

    // Build a single sim/env/parent so genotype & environment are identical
    sim := NewSimulator(5, 1, DEFAULT_MUTABILITY)
    env := NewEnvironment(sim.ImplicitGenome)
    parent := sim.Organisms[0]

    // Baseline: neutral off
    idx = 0 // reset RNG stream
    sim.NeutralRange = 0.0
    countA := 0
    for i := 0; i < 2000; i++ {
        countA += len(parent.OffspringForEnvironment(env))
    }

    // Neutral band on (should not truncate offspring loop)
    idx = 0 // replay exact same RNG stream
    sim.NeutralRange = 0.02
    countB := 0
    for i := 0; i < 2000; i++ {
        countB += len(parent.OffspringForEnvironment(env))
    }

    if countA != countB {
        t.Fatalf("Neutral band changed offspring count (%d vs %d) — check OffspringForEnvironment loop control", countA, countB)
    }
}
